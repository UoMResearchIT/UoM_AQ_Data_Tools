...
# output file names
weather_dataset_out <- "weather_df_Final_comp.csv"
...
weather_df_Final <- read.csv(file=weather_dataset_in,header=TRUE,sep=",")
...
message("MICE: weather")
#   weather
data <- weather_df_Final[,-2]
tempData <- mice(data,m=5,maxit=5,meth='pmm',seed=500)
summary(tempData)
weather_df_Final_comp <- complete(tempData,1)
weather_df_Final_comp$Date <- weather_df_Final$Date


message("writing final datafiles")
# temporarily save DF files #############################################
write.csv(weather_df_Final_comp, file = weather_dataset_out, row.names=F)
# --------------------------------------------------------------------
