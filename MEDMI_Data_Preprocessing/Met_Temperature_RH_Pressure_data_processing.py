...
def load_met_data(file_in,col_list):
    
    print('    load data file')
    met_data = pd.read_csv(file_in,usecols=col_list,skiprows=1,engine='python',sep=',\s*',na_values='None')
...
def find_and_drop_duplicates_and_unwanted_stations(met_data_in,station_list):
...
def drop_single_daily_measurement_stations(met_data_in,print_stats):
...
def station_listing(var_string,met_data_in,min_years=1,reference_num_years=3.5):
    '''    
    arguments:
        var_string:
...
def list_required_and_reference_sites(met_data_in):
...
def rh_calculations(met_data_temp,met_data_dewtemp,print_stats,met_data):
...
def get_full_datasets(imputer,pt,orig_data_in,req_sites_list,useful_sites_list,stations,
...
def organise_data_imputation(met_data_in,start_date_in,end_date_in,stations,\
    ...
    met_data_out_temp = get_full_datasets(imputer,pt,met_data_in,req_sites_temp,reference_sites,stations,
                                                 date_index,reference_station_number,
                                                 station_list,'temperature')
    
    print('imputing pressure data')
    met_data_out_pressure = get_full_datasets(imputer,pt,met_data_in,req_sites_pres,reference_sites,stations,
                                                 date_index,reference_station_number,
                                                 station_list,'pressure')
    
    
    print('imputing dew point temperature')
    met_data_out_dewtemp = get_full_datasets(imputer,pt,met_data_in,req_sites_dewtemp,reference_sites,stations,
                                                 date_index,reference_station_number,
                                                 station_list,'dewtemp')
...
def sort_datasets(orig_data_in,req_sites_list,date_index,var_string):
...
def organise_data(met_data_in,start_date_in,end_date_in,\
...
def remove_low_temperature_data(met_data_in,min_temperature):
...
if __name__ == '__main__':
    ...
    if impute_data:
        ...
        met_data_temp,met_data_pres,met_data_dewtemp = organise_data_imputation(met_data,start_date,end_date,stations,\
                                        reference_sites, req_sites_temp, req_sites_pres, req_sites_dewtemp, \
                                        reference_station_number,imputer,pt)    
    else:
