...
def extract_wrf_data(wrflist,sens):
    ...
    pblh_data = create_dataframe_for_sites(wrflist,sens)


    #%% run the function
    pblh_wrf = getvar(wrflist, "PBLH", timeidx=ALL_TIMES, method="cat")


    for siteID in pblh_data.columns:
        x_index = sens[sens.sensor_name==siteID].xarray.values
        y_index = sens[sens.sensor_name==siteID].yarray.values

        for t_index,dstamp in enumerate(pblh_data.index):
            pblh_data.loc[dstamp][siteID] = pblh_wrf[t_index,y_index,x_index].values.flatten()[0]
...
if __name__ == '__main__':
    ...
    pblh_dataframe = extract_wrf_data(wrflist,sens)

    print('writing to csv file')
    # write to file
    pblh_dataframe.to_csv(out_file_path / 'wrf_pblh_{}.csv'.format(date_range) )
